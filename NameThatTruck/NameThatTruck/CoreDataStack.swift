...
struct CoreDataStack {
    ...
    func addFlickrPhotoToDatabase(urlString: String, truckType: TruckType, fetchedResultsController: NSFetchedResultsController<FlickrPhoto>) {
        let newFlickrPhoto = FlickrPhoto(urlString: urlString, context: fetchedResultsController.managedObjectContext)
        newFlickrPhoto.truckType = truckType
        print("Created new photo: \(String(describing: newFlickrPhoto))")
    }
    
    // MARK: - Fetch truck type
    
    func fetchTruckType(name: String) -> TruckType? {
